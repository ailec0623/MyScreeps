...
var Task = {
    generateTask: function (room) {
        ...
        for(let s in structures){
            ...
            // repair tasks
            if(structures[s].structureType != STRUCTURE_WALL && structures[s].structureType != STRUCTURE_RAMPART && structures[s].hitsMax - structures[s].hits >= 3000){
                let haveTask = 0;
            ...
            if(structures[s].structureType == STRUCTURE_CONTROLLER){
                if(structures[s].my){
                    ...
                    if (haveTask > 0) {
                        ...
                        Releaser.releaseTask(room, 'reserve', structures[s].pos, structures[s].pos, structures[s].id, 1);
                    }
                }
            }
        }
    ...
    generateHarvestProTask: function (room) {
        ...
        var redFlags = _.filter(Game.flags, (flag) => flag.color == COLOR_RED && flag.room && flag.room.name == room.name)
        for (var i in redFlags) {
            ...
            if (haveTask) {
                ...
                Releaser.releaseTask(room, 'harvestpro', redFlags[i].pos, target.pos, target.id, 1);
            }
        }
    },
        ...
        for (var i in targets) {
            ...
            try{
                ...
                if (haveTask > amount / 200) {
                    ...
                    Releaser.releaseTask(room, 'pickup', targets[i].pos, targets[i].pos, targets[i].id, 100 - (amount / 50));
                }
            }catch(e){
    ...
    generateRepairTask: function (room) {
        ...
        var targets = room.find(FIND_STRUCTURES, {
            filter: (structure) => {
                return structure.hitsMax - structure.hits >= 3000 && structure.structureType != STRUCTURE_WALL && structure.structureType != STRUCTURE_RAMPART;
            }
        });
        for (let i in targets) {
            var haveTask = 0;
